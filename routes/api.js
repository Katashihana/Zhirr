...
loghandler = {
    notparam: {
        ...
        getApikey: 'gak punya apikey? chat gw aja yaaa di wa.me/6289626029135 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey invalid, gak punya apikey? chat gw aja yaaa di wa.me/6289626029135 , key nya gratis kok gan, jadi santuyy ajaa'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------AnkerGans'+'ANKER--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'katashi') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'katashi') return res.json(loghandler.invalidKey)
...
router.get('/appstore/search', async (req, res, next) => {
        var apikeyInput = req.query.apikey,
	    search = req.query.search
	...
	if(apikeyInput != 'katashi') return res.json(loghandler.invalidKey)
	if(!search) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter search"})
       fetch(encodeURI(`https://api-yogipw.herokuapp.com/api/search/appstore?query=${search}&num=50&page=1`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
